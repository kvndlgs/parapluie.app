...
const DesktopDropdown = ({
  ...
  return (
    <div className="relative">
      <button
        className="flex items-center space-x-1 px-3 py-2 text-sm text-grape-500 hover:text-primary/80 transition-all"
        onClick={onToggle}
      ...
      >
        ...
        <ChevronDown
          className={`w-4 h-4 text-grape-500 transition-transform duration-200 ${
            isOpen ? "rotate-180" : ""
          }`}
        />
      </button>
        ...
        <div className="py-2 bg-white">
          {item.dropdown?.map((dropdownItem) => (
            <a
              ...
              className="block px-4 py-2 font-sans font-[600] text-sm text-grape-500 hover:text-crime-500 transition-all"
              onClick={onClose}
            >
              {dropdownItem.label}
            </a>
...
const DesktopNavLink = ({ item }: DesktopNavLinkProps) => {
  return (
    <a
      href={item.href}
      className="px-3 py-4 font-sans font-[500] text-xs text-grape-800 hover:text-crime-400 transition-all ease-in-out"
    >
      {item.label}
    </a>
  );
  ...
  return (
    <div className="space-y-1">
      <button
        ...
        className="flex items-center justify-between w-full px-3 py-3 text-base text-grape-500 hover:text-primary-500 transition-all"
      >
        <span className="font-sans font-[500] text-grape-800">
          {" "}
          {item.label}
        </span>
        <ChevronDown
          className={`w-4 h-4 text-grape-800 font-[500] transition-transform duration-200 ${
        ...
        <div className="pl-4 py-2 space-y-1">
          {item.dropdown?.map((dropdownItem) => (
            <a
              ...
              href={dropdownItem.href}
              onClick={onLinkClick}
              className=" block px-3 py-2 font-sans font-[500] text-sm text-grape-800 hover:text-crime-400 transition-all"
            >
              {dropdownItem.label}
            </a>
          ))}
...
const MobileNavLink = ({ item, onLinkClick }: MobileNavLinkProps) => {
  return (
    <a
      href={item.href}
      className="px-3 py-2 font-sans font-[500] text-sm text-grape-800 hover:text-crime-400 transition-all ease-in-out"
    >
      {item.label}
    </a>
  );
...
const MobileMenuToggle = ({ isOpen, onToggle }: MobileMenuToggleProps) => {
  return (
    <button
      onClick={onToggle}
      className="lg:hidden p-2 font-sans font-[500] text-grape-800 hover:text-crime-400  transition-all"
...
export default function Navbar({ className = "" }: NavbarProps) {
  ...
  return (
    <>
      {/* Navbar */}
      <nav  className='relative bg-white rounded-t-xl w-full h-auto z-50 transition-all duration-300'>
        <div className="px-4 md:px-8">
          <div className="flex items-center justify-between h-auto py-2 md:py-6">
            <a href="/">
            <svg
              width="29"
              height="38"
        ...
        >
          <div className="h-auto max-w-[98%] mx-auto space-y-4 py-6 overflow-y-auto">
            {navItems.map((item, index) => (
              <div key={item.label}>
                {item.dropdown ? (
                  <MobileDropdown
      ...
      {/* Mobile Menu Overlay */}
      <div
        className={`lg:hidden fixed py-8 inset-0 bg-black/70 backdrop-blur-md z-20 transition-all duration-300 ${
