...
const ResponsiveNavbar = () => {
  ...
  return (
    ...
    >
      ...
      <div className="hidden md:flex items-center gap-16">
        <div className="hidden md:flex space-x-12  font-medium text-base-750">
          ...
          <a
            ...
            onClick={(e) => scrollToSection(e, "comment-ca-marche")}
            className="hover:text-brand-primary transition-colors"
          >
            Comment ça marche ?
          </a>
        ...
        <div className="hidden md:flex space-x-12  font-medium text-base-750">
          <a
            href="#abonnements"
            onClick={(e) => scrollToSection(e, "abonnements")}
            className="hover:text-brand-primary cursor-pointer transition-colors"
          >
            Abonnements{" "}
          </a>
          <a
            href="https://dashboard.parapluie.app"
            className="hover:text-brand-primary cursor-pointer transition-colors"
          >
            Tableau de bord
          </a>
      ...
      <div
        className="md:hidden cursor-pointer w-8 h-8 flex items-center justify-between md:justify-center transition-transform duration-100"
        onClick={toggleMenu}
      >
        {!isOpen ? (
          <svg
          ...
          >
            <rect
              ...
              height="2"
              rx="1"
              transform="rotate(45 12.136 10.793)"
              fill="#755775"
            />
            <rect
              x="11.136"
              ...
              height="2"
              rx="1"
              transform="rotate(-45 11.136 23.793)"
              fill="#755775"
            />
          </svg>
        )}
      ...
      {isOpen && (
        <div className="md:hidden flex flex-col w-94/100 left-0 right-0 absolute top-20 mx-auto bg-primary-50/97 backdrop-blur-2xl rounded-2xl p-4 shadow-md">
          {menuItems.map((item) => (
            <a key={item.href} href={item.href} className="py-2 text-base-750">
              {item.label}
            </a>
          ))}
        </div>
